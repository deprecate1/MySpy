...
void CMyTreeView::UpdateTree(void)
{
	...
	for (int i = 0; i < pDoc->m_oEnumWins.GetWindowCount(); i++)
	{
		...
		// ===============================================
		CString filter = GetDocument()->GetFilterString();
		BOOL bInsertFlag = TRUE;
		if (!filter.IsEmpty() && NULL == hItem) {
			int pos = strTmp.MakeUpper().Find(filter.MakeUpper());
			if (pos >= 0) {
				bInsertFlag = TRUE;
			} else {
				bInsertFlag = FALSE;
		...
		if (::IsWindow(hWnd) && bInsertFlag)
		{
			...
			//static HTREEITEM tmp = NULL;
			//if (tmp == hItem) {
			//} else {
			//	m_pCtlTree->Expand(hItem, TVE_EXPAND);
			//	tmp == hItem;
			//}
			//XTRACE(L"insertitem: %s\n", strTmp);
		}
	}
	...
	MyExpandTree(m_pCtlTree->GetRootItem());
...
void CMyTreeView::MyExpandTree(HTREEITEM hTreeItem)
{
	if (!m_pCtlTree->ItemHasChildren(hTreeItem))
	{
		return;
	}
	HTREEITEM hNextItem = m_pCtlTree->GetChildItem(hTreeItem);
	while (hNextItem != NULL)
	{
		MyExpandTree(hNextItem);
		hNextItem = m_pCtlTree->GetNextItem(hNextItem, TVGN_NEXT);
	}
	m_pCtlTree->Expand(hTreeItem, TVE_EXPAND);
}
